...
REM PULL IMEI 
FOR /F "skip=1 tokens=4,6 delims= " %%i IN ('adb shell service call iphonesubinfo 1') DO SET mess=%%i,%%j, & <nul set /p =!mess!>>imei_messyboy.txt 
...
REM FOR /F "skip=1 tokens=2,3,4,5,6,7,8,9 delims='.)" %%i IN ('adb shell service call iphonesubinfo 1') DO <NUL SET /P result=%%i%%j%%k%%l%%m%%n%%o%%p>>temp.txt
...
ping -n 1 127.0.0.1 > NUL
cd AP_Logs
echo Starting ADB logging. Please make sure multiple windows opened after this request. If not, check adb settings and device connectivity"
